...
DEFAULT_SCRAPER_TASK_TOPIC = "/dsr01/pharmacy/scraper_task"

ACTIVE_REFILL_ITEM_STATUS = "REFILL_REQUIRED"
READY_ITEM_STATUS = "READY"
SCRAPER_READY_EVENT_STATUS = "WAITING"
...
class TaskManagerBridge(Node):
    ...
    이 노드는 주기적으로:
      1. GET /api/events/
      2. GET /api/medicine/
      3. EventItem.status == REFILL_REQUIRED인 항목만 선택
      4. medicine_name만 사용해 Medicine 매칭
      5. 리필 로봇 제어용 평평한 JSON 배열 publish
      6. Event의 모든 EventItem이 READY이고 Event.status가 WAITING이면
         스크래퍼 작업 토픽 publish
    """
    ...
    def __init__(self):
        ...
        self.scraper_task_topic = (
            self.get_parameter("scraper_task_topic")
            .get_parameter_value()
            .string_value
        )
        ...
        self.scraper_task_pub = self.create_publisher(
            String,
            self.scraper_task_topic,
            10,
        )
        ...
        self._last_scraper_signature = None
        ...
        self.get_logger().info(
            ...
            f"refill_output={self.refill_required_medicine_topic}, "
            f"scraper_output={self.scraper_task_topic}"
        )
    ...
    def _poll_backend(self):
        ...
        refill_tasks = self._build_refill_tasks(events_data, medicines_data)
        self._publish_refill_tasks(refill_tasks)

        scraper_tasks = self._build_scraper_tasks(events_data)
        self._publish_scraper_tasks(scraper_tasks)

    def _publish_refill_tasks(self, tasks):
        if not tasks:
            self._last_task_signature = None
            return
        ...
        # 로봇 노드가 늦게 실행되어도 받을 수 있도록 polling마다 발행한다.
    ...
    def _publish_scraper_tasks(self, tasks):
        if not tasks:
            self._last_scraper_signature = None
            return
        ...
        msg = String()
        msg.data = json.dumps(tasks, ensure_ascii=False)
        self.scraper_task_pub.publish(msg)

        # 스크래퍼가 작업을 수신한 뒤 Event를 PROCESSING으로 변경할 때까지
        # 같은 WAITING Event가 반복 발행될 수 있다. 스크래퍼 노드에서도
        # event_id 기준 중복 방지를 수행해야 한다.
        signature = json.dumps(tasks, ensure_ascii=False, sort_keys=True)
        if signature != self._last_scraper_signature:
            summary = [
                f"event_id={task['event_id']}({task['prescription_name']})"
                for task in tasks
            ]
            self.get_logger().info(
                f"스크래퍼 작업 publish: {len(tasks)}개, {summary}"
            )
            self._last_scraper_signature = signature
    ...
    def _build_scraper_tasks(self, events_data):
        """모든 EventItem이 READY인 WAITING Event를 스크래퍼 작업으로 만든다."""
        events = self._extract_records(events_data)
        scraper_tasks = []
        ...
        for event in events:
            ...
            # READY 여부는 EventItem 전체로 판단한다.
            # WAITING 조건은 PROCESSING/DONE Event의 재실행을 막기 위한 상태 가드다.
            if event_status != SCRAPER_READY_EVENT_STATUS:
                continue
            ...
            if not all(
                ...
                and str(item.get("status", "")).strip().upper()
                == READY_ITEM_STATUS
                for item in items
            ):
                continue
            ...
            scraper_tasks.append(
                {
                    "event_id": event_id,
                    "prescription_name": event.get("prescription_name"),
