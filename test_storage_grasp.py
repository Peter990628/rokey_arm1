...
class StorageGraspTester:
    ...
    def pull_lid_open(self):
        ...
        self.node.get_logger().info("당겨서 여는 뚜껑 오픈 시퀀스 시작")

        # 약통에 병따개 걸 위치로 이동
        self.node.get_logger().info("약통에 병따개 걸 위치로 이동")
        ...
        self.movej(
            self.posj(0, 0, 0, 0, 50, 0), 
            vel=5, acc=5, mod=self.DR_MV_MOD_REL
        )
        self.wait(0.5)

        # 뚜껑 열기 전 힘 제어 및 강성 세팅
        # self.task_compliance_ctrl([10000, 500, 500, 10000, 10000, 10000])
        # self.set_desired_force(fd=[0, 0, 25, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0])
        
        # 평행이동하여 뚜껑 오픈
        # self.movel(
        #     self.posx(0, -30, 40, 0, 0, 0), 
        #     vel=20, acc=10, 
        #     ref=self.DR_BASE, 
        ...
        # self.node.get_logger().info("당겨서 여는 뚜껑 오픈 시퀀스 완료")


    def pull_lid(self):
        self.node.get_logger().info("=== 단위 테스트 시작: 당겨서 여는 약통 뚜껑 열기 ===")
        ...
        self.movel(
            self.posx(0, 0, -30, 0, 0, 0), vel=5, acc=5, ref=self.DR_BASE, mod=self.DR_MV_MOD_REL
        )
        ...
        # 병따개 위로 꺼내기
        self.movel(
            self.posx(0, 0, 120, 0, 0, 0), vel=30, acc=30, ref=self.DR_BASE, mod=self.DR_MV_MOD_REL
        )
        self.wait(0.5)
        ...
        # 병따개 제자리에 가져다 놓기
        self.movejx(
            self.X_OPENER_RETURN, vel=self.vel, acc=self.acc, ref=self.DR_BASE, sol=0
        )

        # 아래로 내려서 병따개 원위치
        self.movel(
            self.posx(0, 0, -100, 0, 0, 0), vel=15, acc=15, ref=self.DR_BASE, mod=self.DR_MV_MOD_REL
        )
        self.wait(0.5)
        ...
        self.node.get_logger().info("=== 단위 테스트 완료 3 ===")
...
def main(args=None):
    ...
    tester = StorageGraspTester(node)
